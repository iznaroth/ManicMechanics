...
public class Connection {
    ...
    private BlockEntity attached; //This should never be wrong, but we'll check against it anyways.
    ...
    public ItemStack getNewItemOutput(){
        IItemHandler handler = this.attached.getCapability(ForgeCapabilities.ITEM_HANDLER).orElse(null);
    ...
    public TubeBundleBE getParentTile(){
        return this.parent;
    }
    ...
    public BlockEntity getAttached(){
    ...
    public int getFirstLegalInsertSlot(ItemStack toInsert){
        IItemHandler handler = this.getAttached().getCapability(ForgeCapabilities.ITEM_HANDLER).orElse(null);
        if(handler == null){
