...
import com.iznaroth.manicmechanics.tools.UsefulFunctions;
...
public class RenderWavefrontObj {
  ...
  public static void renderWavefrontObj(TileRenderTester tileEntityMBE21, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer renderBuffers,
    ...
    if (SCALE_EVENLY_BASED_ON_Y) {
      scaleY = WORLD_MODEL_SIZE.y() / OBJ_MODEL_SIZE.y();
      scaleX = scaleY;
      scaleZ = scaleY;
    } else {
      scaleX = WORLD_MODEL_SIZE.x() / OBJ_MODEL_SIZE.x();
      scaleY = WORLD_MODEL_SIZE.y() / OBJ_MODEL_SIZE.y();
      scaleZ = WORLD_MODEL_SIZE.z() / OBJ_MODEL_SIZE.z();
    }

    // must convert our model space size to world space before translating
    final Vector3d OBJ_MODEL_BOTTOM_APEX_WORLD_SPACE = new Vector3d(OBJ_MODEL_BOTTOM_APEX.x() * scaleX,
            OBJ_MODEL_BOTTOM_APEX.y() * scaleY,
            OBJ_MODEL_BOTTOM_APEX.z() * scaleZ);
    ...
    BlockState state = MMBlocks.RENDER_TESTER.get().defaultBlockState().setValue(BlockRenderTester.USE_WAVEFRONT_OBJ_MODEL, true);
...
//    BlockPos blockPos = tileEntityMBE21.getPos();
   ...
   */
  private static void applyAnimations(TileRenderTester tileEntityMBE21, MatrixStack matrixStack) {
    ...
    if (player != null && blockPos != null) {
      Vector3d pedestalCentre = Vector3d.atLowerCornerOf(blockPos).add(0.5, 1.0, 0.5);
   ...
   */
  private static int getCombinedLight(TileRenderTester tileEntityMBE21, int ambientCombinedLight) {
