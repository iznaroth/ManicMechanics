...
public class ObliterationPlinthRenderer implements BlockEntityRenderer<ObliterationPlinthBlockEntity> {
    ...
    @Override
    public void render(ObliterationPlinthBlockEntity be, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int combinedLight, int combinedOverlay) {
        IItemHandler h = be.getLevel().getCapability(Capabilities.ItemHandler.BLOCK, be.getBlockPos(), null);
        if (h != null) {
            ItemStack stack = h.getStackInSlot(ObliterationPlinthBlockEntity.SLOT);
            if (!stack.isEmpty()) {
